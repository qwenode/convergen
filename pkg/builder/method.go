...
func (p *FunctionBuilder) CreateFunction(m *bmodel.MethodEntry) (*gmodel.Function, error) {
	...
	if m.Opts.Reverse && 0 < len(additionalArgs) {
		return nil, logger.Errorf(
			"%v: reverse cannot be used with additional arguments", p.fset.Position(m.Method.Pos()),
		)
	}
	...
	for _, arg := range additionalArgs {
		if util.IsInvalidType(arg.Type()) {
			return nil, logger.Errorf(
				"%v: arg type is not defined. make sure to be imported", p.fset.Position(arg.Pos()),
			)
		}
	}
